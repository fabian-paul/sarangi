...
def load_matrix(branch, iteration, subdir='colvars', reduction='min'):
    ...
    for key, overlap in matrix.items():
        ...
        symm_matrix[(keys[1], keys[0])] = overlap
    return symm_matrix


def load_pair(image_id_a: str, image_id_b: str, subdir='colvars', reduction=None, try_swapped=True):
    import csv
    ...
    if try_swapped:
        return load_pair(image_id_b, image_id_b, subdir=subdir, reduction=reduction, try_swapped=False)
    else:
        raise RuntimeError('Couldn not find overlap recored for image pair %s %s.'%(image_id_a, image_id_b))
...
def main_overlap(branch, iteration, image_id=None, subdir=None, refresh=False):
    r'''Precomputes the overlap between between one image and all others. Stores one yaml file per image.
    ...
    for subdir_ in subdirs:
        if os.path.isdir(folder_obs + '/' + subdir_) and not '_init' in subdir_:
            ...
            for image in images:
                ...
                if file_is_up_to_date(fname_out, fnames_other_images + [fname_image]) and not refresh:
                     continue  # is results are already on disk and we were not asked to refresh, skip
